...
class Client(ClientBase):
    sync_resources_path = "/sync-resources/"
    conenats_path = "/conenats/"
    conenat_path = "/conenats/%s"
    sharedbandwidth_path = "/sharedbandwidths/%s"
    sharedbandwidths_path = "/sharedbandwidths/"
    sslvpn_user_path = '/sslvpn-users/%s'
    sslvpn_users_path = '/sslvpn-users'
    sslvpn_usergroup_path = "/sslvpn-usergroups/%s"
    sslvpn_usergroups_path = "/sslvpn-usergroups"
    sslvpn_rule_path = "/sslvpn-rules/%s"
    sslvpn_rules_path = "/sslvpn-rules"
    sslvpn_service_path = '/sslvpn-services/%s'
    sslvpn_services_path = '/sslvpn-services'
    ns_qos_policy_path = "/ipv6_ns_qos_policies/%s"
    ns_qos_policies_path = "/ipv6_ns_qos_policies/"
    fiprecord_path = "/fiprecords/%s"
    fiprecords_path = "/fiprecords/"
    dummyfloatingip_path =  "/dummyfloatingips/%s"
    dummyfloatingips_path = "/dummyfloatingips/"
    nat_gateway_dnat_rule_path = "/nat-gateway-dnat-rules/%s"
    nat_gateway_dnat_rules_path = "/nat-gateway-dnat-rules/"
    nat_gateway_snat_rule_path = "/nat-gateway-snat-rules/%s"
    nat_gateway_snat_rules_path = "/nat-gateway-snat-rules/"
    nat_gateway_path = "/nat-gateways/%s"
    nat_gateways_path = "/nat-gateways/"
    l3_dci_connect_path = "/l3-dci-connects/%s"
    l3_dci_connects_path = "/l3-dci-connects"
    l2_dci_connect_path = "/l2-dci-connects/%s"
    l2_dci_connects_path = "/l2-dci-connects"
    vpc_connection_path = "/vpc-connections/%s"
    vpc_connections_path = "/vpc-connections"
    ...
    quota_detail_path = "/quotas/%s/details"
    ...
    lbaas_acl_groups_path = "/lbaas/acl-groups"
    ...
    productinfo_path = '/productinfoes/%s'
    productinfoes_path = '/productinfoes'
    tags_path = "/%s/%s/tags"
    tag_path = "/%s/%s/tags/%s"
    trunks_path = "/trunks"
    trunk_path = "/trunks/%s"
    ...
    logs_path = "/logs"
    log_path = "/logs/%s"
    loggable_resources_path = "/log/loggable_resources"
    loggable_resource_path = "/log/loggable_resources/%s"
    network_logs_path = "/log/logs"
    network_log_path = "/log/logs/%s"
    network_loggables_path = "/log/loggable-resources"

    #ipv6_ns_qos_policies_path = "/ipv6_ns_qos_policies"
    #ipv6_ns_qos_policy_path = "/ipv6_ns_qos_policies/%s"

    port_forwardings_path = "/floatingips/%s/port_forwardings"
    port_forwarding_path = "/floatingips/%s/port_forwardings/%s"
    underlayacls_path = "/underlayacls/"
    underlayacl_path = "/underlayacls/%s"
    underlayacl_rules_path = "/underlayacls/%s/underlayacl_rules"
    underlayacl_rule_path = "/underlayacls/%s/underlayacl_rules/%s"
    vgw_mappings_path = "/vgw_mappings/"
    vgw_mapping_path = "/vgw_mappings/%s"

    # API has no way to report plurals, so we have to hard code them
    EXTED_PLURALS = {'routers': 'router',
                     'sync_resources':'sync_resource',
                     'conenats': 'conenat',
                     'sharedbandwidths':'sharedbandwidth',
                     'sslvpn_users': 'sslvpn_user',
                     'sslvpn_usergroups': 'sslvpn_usergroup',
                     'sslvpn_rules': 'sslvpn_rule',
                     'sslvpn_services': 'sslvpn_service',
                     'ipv6_ns_qos_policies':'ipv6_ns_qos_policy',
                     'fiprecords': 'fiprecord',
                     'dummyfloatingips': 'dummyfloatingip',
                     'nat-gateway-dnat-rules': 'nat-gateway-dnat-rule',
                     'nat-gateway-snat-rules': 'nat-gateway-snat-rule',
                     'nat-gateways':'nat-gateway',
                     'dp_l3_dci_connects': 'dp_l3_dci_connect',
                     'dp_l2_dci_connects': 'dp_l2_dci_connect',
                     'vpc_connections': 'vpc_connection',
    ...
    @debtcollector.renames.renamed_kwarg(
        'tenant_id', 'project_id', replace=True)
    def show_quota_detail(self, project_id, **_params):
        """Fetch information of a certain project's detail quotas."""
        return self.get(self.quota_detail_path % (project_id), params=_params)
    ...
    def create_lbaas_acl_group(self, body=None):
        """Create a LB-ACL group."""
        return self.post(self.lbaas_acl_groups_path, body=body)

    def delete_lbaas_acl_group(self, lbaas_acl_group):
        """Delete the specified LB-ACL group."""
        return self.delete(self.lbaas_acl_group_path % (
            lbaas_acl_group))

    def update_lbaas_acl_group(self, lbaas_acl_group, body=None):
        """Update a LB-ACL group."""
        return self.put(self.lbaas_acl_group_path % (
            lbaas_acl_group), body=body)

    def show_lbaas_acl_group(self, lbaas_acl_group, **_params):
        """Fetch information of a LB-ACL group."""
        return self.get(self.lbaas_acl_group_path % (
            lbaas_acl_group), params=_params)

    def list_lbaas_acl_groups(self, retrieve_all=True, **_params):
        """Fetch list of all the LB-ACL group."""
        return self.list('acl_groups',
                         self.lbaas_acl_groups_path,
                         retrieve_all, **_params)

    def list_acl_groups(self, retrieve_all=True, **_params):
        """Fetch list of all the LB-ACL group."""
        return self.list('acl_groups',
                         self.lbaas_acl_groups_path,
                         retrieve_all, **_params)

    def add_lbaas_acl_to_listener(self, lbaas_acl_group, body=None):
        """Add LB-ACL group to a listener."""
        return self.put((self.lbaas_acl_group_path % lbaas_acl_group)
                        + "/add_listener", body=body)

    def remove_lbaas_acl_from_listener(self, lbaas_acl_group, body=None):
        """Remove LB-ACL group from the specified listener."""
        return self.put((self.lbaas_acl_group_path % lbaas_acl_group)
                        + "/remove_listener", body=body)

    def flush_lbaas_acl_group_rules(self, lbaas_acl_group, body=None):
        """Flush all the LB-ACL rules of a LB-ACL group."""
        return self.put((self.lbaas_acl_group_path % lbaas_acl_group)
                        + "/flush_acl_rules", body=body)

    def create_lbaas_acl_rule(self, lbaas_acl_group, body=None):
        """Create a LB-ACL rule."""
        return self.post(
            self.lbaas_acl_rules_path % lbaas_acl_group, body=body)

    def delete_lbaas_acl_rule(self, lbaas_acl_rule, lbaas_acl_group):
        """Delete the specified LB-ACL rule."""
        return self.delete(self.lbaas_acl_rule_path % (
            lbaas_acl_group, lbaas_acl_rule))

    def update_lbaas_acl_rule(self, lbaas_acl_rule,
                              lbaas_acl_group, body=None):
        """Update a LB-ACL rule."""
        return self.put(self.lbaas_acl_rule_path % (
            lbaas_acl_group, lbaas_acl_rule), body=body)

    def show_lbaas_acl_rule(self, lbaas_acl_rule,
                            lbaas_acl_group, **_params):
        """Fetch information of a LB-ACL rule."""
        return self.get(self.lbaas_acl_rule_path % (
            lbaas_acl_group, lbaas_acl_rule), params=_params)

    def list_lbaas_acl_rules(self, lbaas_acl_group,
                             retrieve_all=True, **_params):
        """Fetch list of all LB-ACL rules of a LB-ACL group."""
        return self.list('acl_rules',
                         self.lbaas_acl_rules_path % lbaas_acl_group,
                         retrieve_all, **_params)
    ...
    def list_productinfoes(self, retrieve_all=True, **_params):
        """Fetches product information for users."""
        return self.list('productinfoes', self.productinfoes_path,
                         retrieve_all, **_params)
    ...
    def create_log(self, body=None):
        """Creates a new log resource."""
        return self.post(self.logs_path, body=body)

    def update_log(self, log, body=None):
        """Updates a log resource."""
        self.put(self.log_path % log, body=body)

    def delete_log(self, log):
        """Deletes the specified Log resource."""
        self.delete(self.log_path % log)

    def list_logs(self, retrieve_all=True, **_params):
        """Fetches a list of all log resources."""
        return self.list('logs', self.logs_path, retrieve_all, **_params)

    def show_log(self, log, **_params):
        """Fetches information of a certain log resource."""
        return self.get(self.log_path % log, params=_params)

    def list_loggable_resources(self, retrieve_all=True, **_params):
        """List available loggable resources."""
        return self.list('loggable_resources', self.loggable_resources_path,
                         retrieve_all, **_params)

    def create_port_forwarding(self, floatingip_id, body=None):
        """Create port forwarding resources of given floatingip."""
        return self.post(self.port_forwardings_path % floatingip_id,
                         body=body)

    def update_port_forwarding(self, port_forwarding, floatingip_id,
                               body=None):
        """Upddate port forwarding resources of given floatingip."""
        return self.put(self.port_forwarding_path %
                        (floatingip_id, port_forwarding), body=body)

    def delete_port_forwarding(self, port_forwarding, floatingip_id):
        """Delete port forwarding resources of given floatingip."""
        return self.delete(self.port_forwarding_path %
                           (floatingip_id, port_forwarding))

    def list_port_forwardings(self, floatingip_id, retrieve_all=True,
                              **_params):
        """Fetches a list of all port forwarding resources of
        given floatingip.

        """
        return self.list('port_forwardings',
                         self.port_forwardings_path % floatingip_id,
                         retrieve_all, **_params)

    def show_port_forwarding(self, port_forwarding, floatingip_id, **_params):
        """Fetches information of a certain port forwarding resource of
        given floatingip.

        """
        return self.get(self.port_forwarding_path %
                        (floatingip_id, port_forwarding), params=_params)
    ...
    def create_underlayacl(self, body=None):
        """Creates a new underlayacl."""
        return self.post(self.underlayacls_path, body=body)

    def update_underlayacl(self, underlayacl, body=None):
        """Updates a underlayacl."""
        return self.put(self.underlayacl_path %
                        underlayacl, body=body)

    def list_underlayacls(self, retrieve_all=True, **_params):
        """Fetches a list of all underlayacls for a project."""
        return self.list('underlayacls', self.underlayacls_path,
                         retrieve_all, **_params)

    def show_underlayacl(self, underlayacl, **_params):
        """Fetches information of a certain underlayacl."""
        return self.get(self.underlayacl_path % underlayacl,
                        params=_params)

    def delete_underlayacl(self, underlayacl):
        """Deletes the specified underlayacl."""
        return self.delete(self.underlayacl_path % (underlayacl))

    def create_underlayacl_rule(self, underlayacl_id, body=None):
        """Creates a new underlayacl_rule."""
        return self.post(self.underlayacl_rules_path % underlayacl_id,
                         body=body)

    def update_underlayacl_rule(self, underlayacl_rule, underlayacl_id,
                                body=None):
        """Updates a underlayacl_rule."""
        return self.put(self.underlayacl_rule_path %
                        (underlayacl_id, underlayacl_rule), body=body)

    def list_underlayacl_rules(self, underlayacl_id, retrieve_all=True,
                               **_params):
        """Fetches a list of all underlayacl_rules for a project."""
        return self.list('underlayacl_rules',
                         self.underlayacl_rules_path % underlayacl_id,
                         retrieve_all, **_params)

    def show_underlayacl_rule(self, underlayacl_rule,
                              underlayacl_id, **_params):
        """Fetches information of a certain underlayacl_rule."""
        return self.get(self.underlayacl_rule_path %
                        (underlayacl_id, underlayacl_rule), params=_params)

    def delete_underlayacl_rule(self, underlayacl_rule, underlayacl_id):
        """Deletes the specified underlayacl_rule."""
        return self.delete(self.underlayacl_rule_path %
                           (underlayacl_id, underlayacl_rule))

    def list_vgw_mappings(self, retrieve_all=True, **_params):
        """Fetches a list of all vgw mapping for a project."""
        return self.list('vgw_mappings', self.vgw_mappings_path,
                         retrieve_all, **_params)

    def show_vgw_mapping(self, vgw_mapping_id, **_params):
        """Fetches information of a certain vgw mapping."""
        return self.get(self.vgw_mapping_path % vgw_mapping_id, params=None)

    def create_vgw_mapping(self, body=None):
        """Creates a new vgw mapping."""
        return self.post(self.vgw_mappings_path, body=body)

    def update_vgw_mapping(self, vgw_mapping_id, body=None):
        """Updates a new vgw mapping."""
        return self.put(self.vgw_mapping_path % vgw_mapping_id, body=body)

    def delete_vgw_mapping(self, vgw_mapping_id):
        """Deletes the specified vgw mapping."""
        return self.delete(self.vgw_mapping_path % vgw_mapping_id)

    def list_vgw_mapping_details(self, retrieve_all=True, **_params):
        """Fetches a list of all vgw mapping for a project."""
        return self.list('vgw_mappings', self.vgw_mappings_path,
                         retrieve_all, **_params)

    def show_vgw_mapping_detail(self, vgw_mapping_id, **_params):
        """Fetches information of a certain vgw mapping."""
        return self.get(self.vgw_mapping_path % vgw_mapping_id, params=None)

    def update_vgw_mapping_detail(self, vgw_mapping_id, body=None):
        """Add vgw mapping details."""
        VGW_MAPPING_DETAIL = 'vgw_mapping_detail'
        MAPPING_DETAIL = 'mapping_detail'
        DETAIL_IDS = 'detail_ids'
        if MAPPING_DETAIL in body[VGW_MAPPING_DETAIL]:
            mapping_detail = body[VGW_MAPPING_DETAIL][MAPPING_DETAIL]
            body = {MAPPING_DETAIL: mapping_detail}
            return self.put(self.vgw_mapping_path % vgw_mapping_id + "/add_mapping_detail",
                            body=body)

        if DETAIL_IDS in body[VGW_MAPPING_DETAIL]:
            detail_ids = body[VGW_MAPPING_DETAIL][DETAIL_IDS]
            body = {DETAIL_IDS: detail_ids}
            return self.put(self.vgw_mapping_path % vgw_mapping_id + "/remove_mapping_detail",
                            body=body)
    ...
    def list_vpc_connections(self, retrieve_all=True, **_params):
        return self.list('vpc_connections', self.vpc_connections_path, retrieve_all, **_params)

    def show_vpc_connection(self, vpc_connection, **_params):
        return self.get(self.vpc_connection_path % (vpc_connection), params=_params)

    def create_vpc_connection(self, body=None):
        return self.post(self.vpc_connections_path, body=body)

    def update_vpc_connection(self, vpc_connection, body=None):
        return self.put(self.vpc_connection_path % (vpc_connection), body=body)

    def delete_vpc_connection(self, vpc_connection):
        return self.delete(self.vpc_connection_path % (vpc_connection))

    def list_l2_dci_connects(self, retrieve_all=True, **_params):
        return self.list('l2_dci_connects', self.l2_dci_connects_path, retrieve_all, **_params)

    def show_l2_dci_connect(self, l2_dci_connect, **_params):
        return self.get(self.l2_dci_connect_path % (l2_dci_connect), params=_params)

    def create_l2_dci_connect(self, body=None):
        return self.post(self.l2_dci_connects_path, body=body)

    def update_l2_dci_connect(self, l2_dci_connect, body=None):
        return self.put(self.l2_dci_connect_path % (l2_dci_connect), body=body)

    def delete_l2_dci_connect(self, l2_dci_connect):
        return self.delete(self.l2_dci_connect_path % (l2_dci_connect))

    def list_l3_dci_connects(self, retrieve_all=True, **_params):
        return self.list('l3_dci_connects', self.l3_dci_connects_path, retrieve_all, **_params)

    def show_l3_dci_connect(self, l3_dci_connect, **_params):
        return self.get(self.l3_dci_connect_path % (l3_dci_connect), params=_params)

    def create_l3_dci_connect(self, body=None):
        return self.post(self.l3_dci_connects_path, body=body)

    def update_l3_dci_connect(self, l3_dci_connect, body=None):
        return self.put(self.l3_dci_connect_path % (l3_dci_connect), body=body)

    def delete_l3_dci_connect(self, l3_dci_connect):
        return self.delete(self.l3_dci_connect_path % (l3_dci_connect))

    def create_nat_gateway(self, body=None):
        return self.post(self.nat_gateways_path, body=body)

    def list_nat_gateways(self, retrieve_all=True, **_params):
        return self.list('nat_gateways', self.nat_gateways_path, retrieve_all, **_params)

    def show_nat_gateway(self, nat_gateway, **_params):
        return self.get(self.nat_gateway_path % (nat_gateway), params=_params)

    def update_nat_gateway(self, nat_gateway, body=None):
        return self.put(self.nat_gateway_path % (nat_gateway), body=body)

    def delete_nat_gateway(self, nat_gateway):
        return self.delete(self.nat_gateway_path % (nat_gateway))


    def create_nat_gateway_snat_rule(self, body=None):
        return self.post(self.nat_gateway_snat_rules_path, body=body)

    def list_nat_gateway_snat_rules(self, retrieve_all=True, **_params):
        return self.list('nat_gateway_snat_rules', self.nat_gateway_snat_rules_path, retrieve_all, **_params)

    def show_nat_gateway_snat_rule(self, nat_gateway_snat_rule, **_params):
        return self.get(self.nat_gateway_snat_rule_path % (nat_gateway_snat_rule), params=_params)

    def update_nat_gateway_snat_rule(self, nat_gateway_snat_rule, body=None):
        return self.put(self.nat_gateway_snat_rule_path % (nat_gateway_snat_rule), body=body)

    def delete_nat_gateway_snat_rule(self, nat_gateway_snat_rule):
        return self.delete(self.nat_gateway_snat_rule_path % (nat_gateway_snat_rule))


    def create_nat_gateway_dnat_rule(self, body=None):
        return self.post(self.nat_gateway_dnat_rules_path, body=body)

    def list_nat_gateway_dnat_rules(self, retrieve_all=True, **_params):
        return self.list('nat_gateway_dnat_rules', self.nat_gateway_dnat_rules_path, retrieve_all, **_params)

    def show_nat_gateway_dnat_rule(self, nat_gateway_dnat_rule, **_params):
        return self.get(self.nat_gateway_dnat_rule_path % (nat_gateway_dnat_rule), params=_params)

    def update_nat_gateway_dnat_rule(self, nat_gateway_dnat_rule, body=None):
        return self.put(self.nat_gateway_dnat_rule_path % (nat_gateway_dnat_rule), body=body)

    def delete_nat_gateway_dnat_rule(self, nat_gateway_dnat_rule):
        return self.delete(self.nat_gateway_dnat_rule_path % (nat_gateway_dnat_rule))

    def create_dummyfloatingip(self, body=None):
        return self.post(self.dummyfloatingips_path, body=body)

    def create_fiprecord(self, body=None):
        return self.post(self.fiprecords_path, body=body)

    def list_fiprecords(self, retrieve_all=True, **_params):
        return self.list('fiprecords', self.fiprecords_path, retrieve_all, **_params)

    def show_fiprecord(self, fiprecord, **_params):
        return self.get(self.fiprecord_path % (fiprecord), params=_params)

    def delete_fiprecord(self, fiprecord):
        return self.delete(self.fiprecord_path % (fiprecord))

    def create_ipv6_ns_qos_policy(self, body=None):
        return self.post(self.ns_qos_policies_path, body=body)

    def list_ipv6_ns_qos_policies(self, retrieve_all=True, **_params):
        return self.list('ipv6_ns_qos_policies', self.ns_qos_policies_path, retrieve_all, **_params)

    def show_ipv6_ns_qos_policy(self, ipv6_ns_qos_policy, **_params):
        return self.get(self.ns_qos_policy_path % (ipv6_ns_qos_policy), params=_params)

    def update_ipv6_ns_qos_policy(self, ipv6_ns_qos_policy, body=None):
        return self.put(self.ns_qos_policy_path % (ipv6_ns_qos_policy), body=body)

    def delete_ipv6_ns_qos_policy(self, ipv6_ns_qos_policy):
        return self.delete(self.ns_qos_policy_path % (ipv6_ns_qos_policy))

    def create_sslvpn_service(self, body=None):
        """Create a new service"""
        return self.post(self.sslvpn_services_path, body=body)

    def list_sslvpn_services(self, retrieve_all=True, **_params):
        """Fetches a list of all services for a project."""
        return self.list('sslvpn_services', self.sslvpn_services_path,
                         retrieve_all, **_params)

    def show_sslvpn_service(self, sslvpn_service, **_params):
        """Fetches information for a service."""
        return self.get(self.sslvpn_service_path % (sslvpn_service),
                        params=_params)

    def update_sslvpn_service(self, sslvpn_service, body=None):
        """Updates a service."""
        return self.put(self.sslvpn_service_path % (sslvpn_service),
                        body=body)

    def delete_sslvpn_service(self, sslvpn_service):
        """Deletes the specified service."""
        return self.delete(self.sslvpn_service_path %
                           (sslvpn_service))

    def create_sslvpn_rule(self, body=None):
        """Create a new rule"""
        return self.post(self.sslvpn_rules_path, body=body)

    def list_sslvpn_rules(self, retrieve_all=True, **_params):
        """Fetches a list of all rules for a project."""
        return self.list('sslvpn_rules', self.sslvpn_rules_path,
                         retrieve_all, **_params)

    def show_sslvpn_rule(self, sslvpn_rule, **_params):
        """Fetches information for a rule."""
        return self.get(self.sslvpn_rule_path % (sslvpn_rule),
                        params=_params)

    def update_sslvpn_rule(self, sslvpn_rule, body=None):
        """Updates a rule."""
        return self.put(self.sslvpn_rule_path % (sslvpn_rule),
                        body=body)

    def delete_sslvpn_rule(self, sslvpn_rule):
        """Deletes the specified rule."""
        return self.delete(self.sslvpn_rule_path %
                           (sslvpn_rule))

    def create_sslvpn_usergroup(self, body=None):
        """Create a new usergroup"""
        return self.post(self.sslvpn_usergroups_path, body=body)

    def list_sslvpn_usergroups(self, retrieve_all=True, **_params):
        """Fetches a list of all usergroups for a project."""
        return self.list('sslvpn_usergroups', self.sslvpn_usergroups_path,
                         retrieve_all, **_params)

    def show_sslvpn_usergroup(self, sslvpn_usergroup, **_params):
        """Fetches information for a usergroup."""
        return self.get(self.sslvpn_usergroup_path % (sslvpn_usergroup),
                        params=_params)

    def update_sslvpn_usergroup(self, sslvpn_usergroup, body=None):
        """Updates a usergroup."""
        return self.put(self.sslvpn_usergroup_path % (sslvpn_usergroup),
                        body=body)

    def delete_sslvpn_usergroup(self, sslvpn_usergroup):
        """Deletes the specified usergroup."""
        return self.delete(self.sslvpn_usergroup_path %
                           (sslvpn_usergroup))

    def create_sslvpn_user(self, body=None):
        """Create a new user"""
        return self.post(self.sslvpn_users_path, body=body)

    def list_sslvpn_users(self, retrieve_all=True, **_params):
        """Fetches a list of all users for a project."""
        return self.list('sslvpn_users', self.sslvpn_users_path,
                         retrieve_all, **_params)

    def show_sslvpn_user(self, sslvpn_user, **_params):
        """Fetches information for a user."""
        return self.get(self.sslvpn_user_path % (sslvpn_user),
                        params=_params)

    def update_sslvpn_user(self, sslvpn_user, body=None):
        """Updatea user."""
        return self.put(self.sslvpn_user_path % (sslvpn_user),
                        body=body)

    def delete_sslvpn_user(self, sslvpn_user):
        """Deletes the specified user."""
        return self.delete(self.sslvpn_user_path %
                           (sslvpn_user))

    def create_sharedbandwidth(self, body=None):
        return self.post(self.sharedbandwidths_path, body=body)

    def list_sharedbandwidths(self, retrieve_all=True, **_params):
        return self.list('sharedbandwidths', self.sharedbandwidths_path, retrieve_all, **_params)

    def show_sharedbandwidth(self, sharedbandwidth, **_params):
        return self.get(self.sharedbandwidth_path % (sharedbandwidth), params=_params)

    def update_sharedbandwidth(self, sharedbandwidth, body=None):
        return self.put(self.sharedbandwidth_path % (sharedbandwidth), body=body)

    def delete_sharedbandwidth(self, sharedbandwidth):
        return self.delete(self.sharedbandwidth_path % (sharedbandwidth))

    def create_conenat(self, body=None):
        return self.post(self.conenats_path, body=body)

    def list_conenats(self, retrieve_all=True, **_params):
        return self.list('conenats', self.conenats_path, retrieve_all, **_params)

    def show_conenat(self, conenat, **_params):
        return self.get(self.conenat_path % (conenat), params=_params)

    def delete_conenat(self, conenat):
        return self.delete(self.conenat_path % (conenat))
