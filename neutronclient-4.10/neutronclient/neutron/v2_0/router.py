...
class SetGatewayRouter(neutronV20.NeutronCommand):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_('Desired IP and/or subnet on external network: '
                   ...
                   'You can repeat this option.'))
        parser.add_argument(
            '--qos-policy', dest='qos_policy',
            help=_('Set gateway ip qos policy.')
        )
        parser.add_argument(
            '--no-qos-policy', dest='no_qos_policy',
            action='store_true',
            help=_('Unset gateway ip qos policy.')
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.qos_policy:
            ...
            router_dict['qos_policy_id'] = _qos_policy_id
        if parsed_args.no_qos_policy:
            router_dict['qos_policy_id'] = None
