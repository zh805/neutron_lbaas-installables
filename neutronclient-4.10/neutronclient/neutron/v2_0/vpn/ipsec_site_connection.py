...
class IPsecSiteConnectionMixin(object):
    ...
    def add_known_arguments(self, parser, is_create=True):
        ...
        parser.add_argument(
            '--bandwidth',
            default='0' if is_create else argparse.SUPPRESS,
            help=_('Set a bandwidth to the connection.'))
        parser.add_argument(
            '--description',
        ...
        parser.add_argument(
            '--local-id',
            required=is_create,
            help=_('Local router identity for authentication. Can be '
                   'IPv4/IPv6 address, e-mail address, key id, or FQDN.'))
        parser.add_argument(
            '--peer-address',
            required=is_create,
