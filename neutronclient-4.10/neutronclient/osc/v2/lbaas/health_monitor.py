...
HTTP_METHODS = ['GET', 'POST', 'DELETE', 'PUT', 'HEAD', 'OPTIONS', 'PATCH',
                'CONNECT', 'TRACE']
TYPE_CHOICES = ['PING', 'HTTP', 'TCP', 'UDP', 'HTTPS', 'TLS-HELLO',
                'UDP-CONNECT']
...
class CreateHealthMonitor(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            '--project',
            metavar='<project>',
            help="Project for the health monitor (name or ID)."
        )
        parser.add_argument(
            '--delay',
            metavar='<delay>',
    ...
    def take_action(self, parsed_args):
        ...
        data = client.create_lbaas_healthmonitor(body=body)

        formatters = {'pools': v2_utils.format_list}

        return (
            rows,
            (v2_utils.get_dict_properties(data['healthmonitor'],
                                          rows,
...
class DeleteHealthMonitor(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.neutronclient.delete_lbaas_healthmonitor(
            health_monitor_id)
...
class ListHealthMonitor(lister.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        attrs = v2_utils.get_health_monitor_attrs(self.app.client_manager,
                                                  parsed_args)
        data = self.app.client_manager.neutronclient.list_lbaas_healthmonitors(
            **attrs)
...
class ShowHealthMonitor(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        data = self.app.client_manager.neutronclient.show_lbaas_healthmonitor(
            health_monitor_id)
        formatters = {'pools': v2_utils.format_list}
...
class SetHealthMonitor(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.client_manager.neutronclient.update_lbaas_healthmonitor(
