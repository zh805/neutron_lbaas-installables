...
class ACLRule(model_base.BASEV2, model_base.HasId, model_base.HasProject):
    ...
    ip_version = sa.Column(sa.Enum(*lb_const.ACL_IP_VERSIONS,
                                   name="acl_rule_ip_versions"),
                           nullable=False)
    acl_group_id = sa.Column(sa.String(36),
                             sa.ForeignKey(
                                 "lbaas_acl_groups.id"),
                             nullable=False)
    ...
    def to_api_dict(self):
        ...
        ret_dict = to_dict(self, [
            'tenant_id', 'id', 'ip_address', 'description', 'ip_version', 'acl_group_id'])
...
class ACLGroupListenerBinding(model_base.BASEV2):
    ...
    __tablename__ = "lbaas_acl_group_listener_bindings"

    listener_id = sa.Column(sa.String(36),
                            sa.ForeignKey(
                                "lbaas_listeners.id", ondelete='CASCADE'),
                            primary_key=True,
                            nullable=False)
    acl_group_id = sa.Column(sa.String(36),
                             sa.ForeignKey("lbaas_acl_groups.id"),
                             nullable=False)
    type = sa.Column(sa.Enum(*lb_const.ACL_CONTROL_TYPES,
...
class ACLGroup(model_base.BASEV2, model_base.HasId, model_base.HasProject):
    ...
    def to_api_dict(self):
        ...
        ret_dict = to_dict(self, [
            'id', 'tenant_id', 'name', 'description', 'region'])
...
class Listener(model_base.BASEV2, model_base.HasId, model_base.HasProject):
    ...
    redirect_up = sa.Column(sa.Boolean(), nullable=False)
    ...
    ca_container_id = sa.Column(sa.String(128), nullable=True)
