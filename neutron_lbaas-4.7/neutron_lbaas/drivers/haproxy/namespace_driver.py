...
from neutron_lbaas.drivers.haproxy import iptables_manager
...
class HaproxyNSDriver(agent_device_driver.AgentDeviceDriver):
    ...
    def create(self, loadbalancer):
        ...
        self._clear_tproxy(namespace)
        self._config_tproxy(namespace)
    ...
    def _clear_tproxy(self, namespace):

        ip_wrapper = ip_lib.IPWrapper(namespace=namespace)
        ip_cmd = {
            'ip_rule_cmd': ['ip', 'rule', 'del', 'fwmark', '1',
                            'lookup', '100'],
            'ip_route_cmd': ['ip', 'route', 'del', 'local',
                             '0.0.0.0/0',
                             'dev', 'lo', 'table', '100']}
        for key in ip_cmd:
            ip_wrapper.netns.execute(ip_cmd[key], check_exit_code=False)

        ipt_mgr = iptables_manager.IptablesManager(namespace=namespace)

        prerouting_rule = ['-p', 'tcp', '-m', 'socket', '-j', CHAIN_DIVERT]
        ipt_mgr.del_rule(table=TABLE_MANGLE, chain='PREROUTING',
                         rule=prerouting_rule)

        set_fwmark_rule = ['-j', 'MARK', '--set-mark', '1']
        ipt_mgr.del_rule(table=TABLE_MANGLE, chain=CHAIN_DIVERT,
                         rule=set_fwmark_rule)

        divert_accept_rule = ['-j', 'ACCEPT']
        ipt_mgr.del_rule(table=TABLE_MANGLE, chain=CHAIN_DIVERT,
                         rule=divert_accept_rule)

        ipt_mgr.del_chain(table=TABLE_MANGLE, chain=CHAIN_DIVERT)

    def _config_tproxy(self, namespace):

        ip_wrapper = ip_lib.IPWrapper(namespace=namespace)
        ip_cmd = {
            'ip_rule_cmd': ['ip', 'rule', 'add', 'fwmark', '1',
                            'lookup', '100'],
            'ip_route_cmd': ['ip', 'route', 'add', 'local',
                             '0.0.0.0/0',
                             'dev', 'lo', 'table', '100']}
        for key in ip_cmd:
            ip_wrapper.netns.execute(ip_cmd[key], check_exit_code=False)
        ...
        ipt_mgr.add_chain(table=TABLE_MANGLE, chain=CHAIN_DIVERT)

        prerouting_rule = ['-p', 'tcp', '-m', 'socket', '-j', CHAIN_DIVERT]
        ipt_mgr.add_rule(table=TABLE_MANGLE, chain='PREROUTING',
                         rule=prerouting_rule)

        set_fwmark_rule = ['-j', 'MARK', '--set-mark', '1']
        ipt_mgr.add_rule(table=TABLE_MANGLE, chain=CHAIN_DIVERT,
                         rule=set_fwmark_rule)
