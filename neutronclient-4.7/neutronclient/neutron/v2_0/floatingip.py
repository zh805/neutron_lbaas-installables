...
class CreateFloatingIP(neutronV20.CreateCommand):
    ...
    def add_known_arguments(self, parser):
        ...
        parser.add_argument(
            '--qos-policy',
            help=_('Qos policy name or id to be associated with the '
                   'Floating IP.'))
    ...
    def args2body(self, parsed_args):
        ...
        if parsed_args.qos_policy:
            _qos_policy_id = neutronV20.find_resourceid_by_name_or_id(
                self.get_client(), 'policy', parsed_args.qos_policy,
                cmd_resource='qos_policy')
            body.update({'qos_policy_id': _qos_policy_id})
...
class AssociateFloatingIP(neutronV20.NeutronCommand):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            '--no-qos-policy',
            action='store_true',
            help=_('Disassociate qos policy for the Floating IP'))
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.qos_policy:
            _qos_policy_id = neutronV20.find_resourceid_by_name_or_id(
                self.get_client(), 'policy', parsed_args.qos_policy,
                cmd_resource='qos_policy')
            update_dict['qos_policy_id'] = _qos_policy_id
        if 'no_qos_policy'in parsed_args and parsed_args.no_qos_policy:
            update_dict['qos_policy_id'] = None
...
class DisassociateFloatingIP(neutronV20.NeutronCommand):
    ...
    def take_action(self, parsed_args):
        neutron_client = self.get_client()
        update_dict = {'floatingip': {'port_id': None}}
        if 'no_qos_policy' in parsed_args and parsed_args.no_qos_policy:
            update_dict['floatingip'].update({'qos_policy_id': None})
        neutron_client.update_floatingip(parsed_args.floatingip_id,
                                         update_dict)
